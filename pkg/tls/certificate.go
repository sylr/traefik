...
func CompareX509TimeBoundaries(origin *x509.Certificate, target *x509.Certificate) bool {
	...
	if (originIsValid && targetIsValid) || (!originIsValid && !targetIsValid) {
		// Either both certs are valid, or both are invalid. In both case we choose
		// the one with the greatest future time limit boundary (greatest NotAfter)
		if target.NotAfter.After(origin.NotAfter) {
			return true
		} else {
			return false
		}
